...
class SQLTool:
    ...
    def build_sql(self, query: SQLQuery) -> tuple[str, dict]:
        ...
        if query.team:
            base += " JOIN players p2 ON p2.id = stats.player_id"
            conditions.append("p2.team = :team")
        ...
        final_sql = f"{base}{where_clause} LIMIT {self.row_limit}"
        return final_sql, params

    def run_query(self, natural_query: str) -> SQLResult:
        parsed = SQLQuery(natural_query=natural_query)
        ...
        if logfire:
            logfire.info("sql_tool.run_query", sql=sql, latency_ms=latency_ms, rows=len(rows))
        result_rows = [SQLResultRow(columns=list(row.keys()), values=list(row)) for row in rows]
